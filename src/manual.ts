...
const generateImportId = (
  ...
  const occurrence = seenIds[baseHash] ?? 0;
  seenIds[baseHash] = occurrence + 1;

  return hasha([baseHash, `${occurrence}`], { algorithm: 'sha256' }).substr(0, 36);
};
...
export const getAmazonTransactions = async function* (
  aoa: AmazonOrderAccess,
): AsyncGenerator<AmazonTransaction> {
  ...
  try {
    for await (const item of aoa.getItems({
    ...
    for await (const item of aoa.getRefunds({
      startDate: new Date(config.startDate),
      endDate: new Date(),
    })) {
    ...
    for await (const item of aoa.getShipments({
      startDate: new Date(config.startDate),
      endDate: new Date(),
    })) {
